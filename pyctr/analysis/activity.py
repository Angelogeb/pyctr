...
# TODO(mdanatg): Add support for PY3 (e.g. Param vs arg).
...
class Scope(object):
  ...
  Note - simple statements may never delete and modify a symbol at the same time.
  However, compound ones like if statements can. In that latter case, it's
  undefined whether the symbol is actually modified or deleted upon
  statement exit. Certain analyses like reaching definitions need to be
  careful about this.
  ...
  def __init__(self, parent, isolated=True, add_unknown_symbols=False):
    ...
    Args:
      ...
      isolated: Whether the scope is isolated, that is, whether variables
        ...
        scope.
      add_unknown_symbols: Whether to handle attributed and subscripts without
        having first seen the base name. E.g., analyzing the statement 'x.y = z'
        without first having seen 'x'.
    """
...
class ActivityAnalyzer(transformer.Base):
  ...
  def _is_simple_Attribute(self, node):
    if not isinstance(node, gast.Attribute):
      return False

    return isinstance(node.value, gast.Name)

  def _is_overloaded_assign(self, node):
    if not self.overload:
      return False

    if not self._is_simple_Attribute(node.func):
      return False

    is_overload_symbol = node.func.value.id == self.overload.symbol_name
    is_assign = node.func.attr == 'assign'
    return is_overload_symbol and is_assign
...
# TODO(jmd1011): Add overload to context object
def resolve(node, context, parent_scope=None, overload=None):
  return ActivityAnalyzer(context, parent_scope, overload=overload).visit(node)
